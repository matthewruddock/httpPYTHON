...
class Server:
    ...
    def __init__(self): #def __init__(self, blacklisted_ips=False, blacklist_websites=False):
        ...
        print("IP ADDRESS TO BE BLOCKED: ",ipList)
        print("URLS TO BE BLOCKED: ",urlList)
        #blacklisted_ips= ['127.0.0.81']
        #blacklist_websites = ['facebook','jamaicaobserver']
        self.max_conn = 0
        self.buffer_size = 0
        self.socket = 0
        self.port = 0
    ...
    def start_server(self, conn=5, buffer=4096, port=8000):
        try:
            self.write_log(self.getTimeStampp() + "   \n\nStarting Server\n\n")
            threading.Thread(target=self.listen(conn, buffer, port)).start()
